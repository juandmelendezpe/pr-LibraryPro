...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
    ...
    // Método de búsqueda personalizada para nombre, email o teléfono
     @org.springframework.data.jpa.repository.Query("SELECT u FROM Usuario u WHERE LOWER(u.nombre) "
    		+ "LIKE LOWER(CONCAT('%', :termino, '%')) "
    		+ "OR u.email = :termino OR u.telefono = :termino")
